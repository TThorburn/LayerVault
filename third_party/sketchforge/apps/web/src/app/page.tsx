...
export default function Home() {
  ...
  useEffect(() => {
    const expectedOrigin = new URLSearchParams(window.location.search).get("layervaultOrigin");
    if (!expectedOrigin || window.parent === window) return;

    const receiveLayerVaultModel = (event: MessageEvent) => {
      if (event.source !== window.parent || event.origin !== expectedOrigin) return;
      const message = event.data as { type?: unknown; name?: unknown; mimeType?: unknown; buffer?: unknown } | null;
      if (!message || message.type !== "layervault:import-model" || typeof message.name !== "string" || !(message.buffer instanceof ArrayBuffer)) return;
      const fileName = message.name.slice(0, 240);
      ...
      void importFilesFromDashboard([file]).then(() => {
        window.parent.postMessage({ type: "sketchforge:layervault-import-received", name: fileName }, expectedOrigin);
      });
    };

    window.addEventListener("message", receiveLayerVaultModel);
    return () => window.removeEventListener("message", receiveLayerVaultModel);
  }, [importFilesFromDashboard]);
