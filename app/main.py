...
STORAGE_CONTROL_DIR = STORAGE_CONFIG_FILE.parent if STORAGE_CONFIG_FILE else None
STORAGE_WEBHOOK_FILE = STORAGE_CONTROL_DIR / "portainer-webhook.json" if STORAGE_CONTROL_DIR else None
STORAGE_PENDING_FILE = STORAGE_CONTROL_DIR / "storage-pending.json" if STORAGE_CONTROL_DIR else None
...
def _portainer_webhook_settings() -> dict[str, Any]:
    if not STORAGE_WEBHOOK_FILE or not STORAGE_WEBHOOK_FILE.is_file():
        return {}
    try:
        payload = json.loads(STORAGE_WEBHOOK_FILE.read_text(encoding="utf-8"))
        return payload if isinstance(payload, dict) else {}
    except (OSError, ValueError):
        return {}


def _portainer_webhook_url() -> str:
    return str(_portainer_webhook_settings().get("url", "")).strip()


def _validate_portainer_webhook_url(value: str) -> str:
    value = str(value or "").strip()
    parsed = urlparse(value)
    if parsed.scheme not in {"http", "https"} or not parsed.netloc or "/api/stacks/webhooks/" not in parsed.path:
        raise ValueError("Paste the stack webhook URL generated by Portainer.")
    if parsed.username or parsed.password:
        raise ValueError("Webhook URLs containing embedded usernames or passwords are not accepted.")
    return value


@app.post("/api/settings/storage/portainer-webhook", dependencies=[Depends(auth)])
def save_portainer_storage_webhook(payload: dict[str, Any]):
    if not STORAGE_WEBHOOK_FILE:
        raise HTTPException(409, "Persistent storage control is not enabled for this deployment.")
    value = str(payload.get("url", "")).strip()
    if not value:
        STORAGE_WEBHOOK_FILE.unlink(missing_ok=True)
        return {"ok": True, "configured": False}
    try:
        value = _validate_portainer_webhook_url(value)
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc
    STORAGE_WEBHOOK_FILE.parent.mkdir(parents=True, exist_ok=True)
    temporary = STORAGE_WEBHOOK_FILE.with_suffix(".tmp")
    temporary.write_text(json.dumps({"url": value, "allow_insecure_tls": bool(payload.get("allow_insecure_tls"))}, indent=2) + "\n", encoding="utf-8")
    temporary.replace(STORAGE_WEBHOOK_FILE)
    try:
        os.chmod(STORAGE_WEBHOOK_FILE, 0o600)
    except OSError:
        pass
...
def _apply_storage_job(destinations: dict[str, Path], host_paths: dict[str, str]) -> None:
    try:
        ...
        if STORAGE_PENDING_FILE:
            STORAGE_PENDING_FILE.unlink(missing_ok=True)
...
def _trigger_portainer_storage_remount(host_paths: dict[str, str]) -> None:
    try:
        webhook_settings = _portainer_webhook_settings()
        webhook = _validate_portainer_webhook_url(str(webhook_settings.get("url", "")))
        variables = {
            "LAYERVAULT_STORAGE_ROOT_1": host_paths["workspace"],
            "LAYERVAULT_STORAGE_ROOT_2": host_paths["database"],
            "LAYERVAULT_STORAGE_ROOT_3": host_paths["models"],
            "LAYERVAULT_STORAGE_ROOT_4": host_paths["backups"],
        }
        separator = "&" if "?" in webhook else "?"
        request = UrlRequest(webhook + separator + urlencode(variables), data=b"", method="POST")
        _set_storage_apply_status(state="remounting", progress=5, message="Portainer is mounting the new locations and redeploying LayerVault…")
        tls_context = ssl._create_unverified_context() if webhook_settings.get("allow_insecure_tls") and webhook.startswith("https://") else None
        with urlopen(request, timeout=30, context=tls_context) as response:
            if response.status >= 400:
                raise RuntimeError(f"Portainer returned HTTP {response.status}.")
    except Exception as exc:
        _set_storage_apply_status(state="failed", progress=0, message=f"Portainer redeploy failed: {exc}", restart_scheduled=False)


def _resume_pending_storage_move() -> None:
    if not STORAGE_PENDING_FILE or not STORAGE_PENDING_FILE.is_file():
        return
    try:
        host_paths = json.loads(STORAGE_PENDING_FILE.read_text(encoding="utf-8"))
        destinations = {key: _translate_storage_path(str(host_paths[key]))[0] for key in ("workspace", "database", "models", "backups")}
    except Exception as exc:
        _set_storage_apply_status(state="failed", progress=0, message=f"The new locations were not mounted after redeploy: {exc}")
        return
    _set_storage_apply_status(state="copying", progress=8, message="New locations mounted. Starting the safe data copy…")
    threading.Thread(target=_apply_storage_job, args=(destinations, host_paths), name="layervault-storage-resume", daemon=True).start()


@app.on_event("startup")
def resume_pending_storage_move() -> None:
    _resume_pending_storage_move()
...
def apply_storage_settings(payload: dict[str, Any]):
    webhook = _portainer_webhook_url()
    if not STORAGE_CONFIG_FILE or (not _storage_roots() and not webhook):
        raise HTTPException(409, "Storage Apply is not enabled for this deployment.")
    with _storage_apply_lock:
        if _storage_apply_status.get("state") in {"validating", "copying", "remounting"}:
    ...
    host_paths = {key: _normalise_host_path(str(payload.get(key, ""))) for key in ("workspace", "database", "models", "backups")}
    try:
        for host_path in host_paths.values():
            if not host_path or _blocked_system_path(host_path) or ".." in host_path.replace("\\", "/").split("/"):
                raise ValueError("Choose data folders or mounted shares, not operating-system folders.")
        for key in ("workspace", "database", "models", "backups"):
            destination, _ = _translate_storage_path(host_paths[key])
            destination.mkdir(parents=True, exist_ok=True)
        ...
        if webhook and "outside the storage roots" in str(exc):
            assert STORAGE_PENDING_FILE is not None
            STORAGE_PENDING_FILE.parent.mkdir(parents=True, exist_ok=True)
            temporary = STORAGE_PENDING_FILE.with_suffix(".tmp")
            temporary.write_text(json.dumps(host_paths, indent=2) + "\n", encoding="utf-8")
            temporary.replace(STORAGE_PENDING_FILE)
            worker = threading.Thread(target=_trigger_portainer_storage_remount, args=(host_paths,), name="layervault-portainer-remount", daemon=True)
            worker.start()
            return {"ok": True, "state": "remounting", "message": "Portainer is redeploying LayerVault with the new storage mounts."}
...
def backup_settings():
    ...
    return {
        ...
        "storage": {
            ...
            "portainer_webhook_configured": bool(_portainer_webhook_url()),
            "portainer_allow_insecure_tls": bool(_portainer_webhook_settings().get("allow_insecure_tls")),
        },
    }
